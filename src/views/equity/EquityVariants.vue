    ...
    <BaseDialog v-model="settingsVisible" class="settings-dialog equity-settings-dialog" width="700px" :show-close="false" :close-on-click-modal="true" :close-on-press-escape="false" destroy-on-close>
      ...
      <section v-if="settingsDetail === 'password'" class="settings-password-view">
        <form class="password-change-form" @submit.prevent="confirmPasswordChange">
          ...
          <label>新密码<el-input v-model="passwordForm.password" :class="{ 'is-error': passwordFieldErrors.password }" type="password" show-password autocomplete="new-password" placeholder="请输入新密码" @blur="validateNewPassword()" @input="passwordForm.passwordConfirmation && validatePasswordConfirmation()" /><small>密码须包含英文字母与特殊符号，且长度不少于 8 位。</small><p v-if="passwordFieldErrors.password" class="password-field-error" role="alert">{{ passwordFieldErrors.password }}</p></label>
