...
const props = defineProps({ instruments: Array, accounts: Array, symbol: Object, account: Object, quote: Object, paused: Boolean, closeMode: Boolean, ticketContext: Object })
const emit = defineEmits(['select', 'account-select', 'order'])
const executionType = ref(props.ticketContext?.executionType === 'highTouch' ? 'highTouch' : 'lowTouch'), algorithm = ref(['POV', 'DMA'].includes(props.ticketContext?.algorithm) ? props.ticketContext.algorithm : 'DMA'), algorithmPopoverVisible = ref(false), orderType = ref('limit'), price = ref(), quantity = ref(), amount = ref(), quantityMode = ref('quantity'), fraction = ref(0), note = ref('')
const symbolMarket = ref('ALL'), search = ref(''), unit = ref('shares'), amountUnit = ref('yuan'), confirming = ref(false), insufficientFunds = ref(false), quantityLimitError = ref(''), snapshot = ref(null)
...
const commonValid = computed(() => !!displaySymbolCode.value && shares.value > 0 && shares.value % (lotSize.value || 100) === 0 && (orderType.value === 'market' || price.value > 0))
...
function chooseSymbol(code) {
  ...
  rememberRecent(code)
  resetOrder()
  if (instrument.isManual) { selectedManualInstrument.value = instrument; price.value = undefined; return }
...
function openManualInstrument() { if (!isManualOrder.value) return; manualInstrumentName.value = ''; manualInstrumentCode.value = ''; manualInstrumentError.value = ''; manualInstrumentVisible.value = true }
  ...
  <div class="order-form" :class="{ 'is-paused': paused, 'is-manual': !closeMode && executionType === 'highTouch' }" :inert="paused ? '' : undefined" :aria-disabled="paused">
    ...
    <section v-if="!closeMode" class="order-context-block">
      <div class="execution-type-block">
        ...
        <el-popover v-model:visible="algorithmPopoverVisible" trigger="click" placement="bottom-end" :width="164" popper-class="variant-popper algorithm-popper"><template #reference><button type="button" class="algorithm-trigger" :aria-label="`执行算法：${algorithm}，点击选择`"><span>算法·{{ algorithm }}</span><svg class="algorithm-chevron" :class="{ 'is-open': algorithmPopoverVisible }" viewBox="0 0 12 12" fill="none" aria-hidden="true"><path d="m3 4.5 3 3 3-3" /></svg></button></template><div class="execution-algorithm-menu"><span>执行算法</span><el-radio-group v-model="algorithm" aria-label="执行算法" @change="algorithmPopoverVisible = false"><el-radio-button label="POV">POV</el-radio-button><el-radio-button label="DMA">DMA</el-radio-button></el-radio-group></div></el-popover>
      </div>
    ...
    <section class="quantity-block">
      ...
      <template v-if="quantityMode === 'quantity'"><div class="quantity-input-row" :class="{ 'is-error': lotInvalid || quantityLimitError }"><el-input-number :key="quantityInputKey" id="variant-quantity" v-model="quantity" placeholder="请输入" :step="unit === 'wan' ? 1 : (lotSize || 100)" :precision="0" :controls="false" /><el-select v-model="unit" aria-label="数量单位" popper-class="variant-popper quantity-unit-popper" placement="bottom-end" :offset="4" :class="{ 'is-wan-unit': unit === 'wan' }"><el-option label="股" value="shares" /><el-option label="万股" value="wan" /></el-select></div><span v-if="lotInvalid || quantityLimitError" class="lot-error" role="alert">{{ lotInvalid ? '订单数量须为整手，请重新输入' : quantityLimitError }}</span></template>
