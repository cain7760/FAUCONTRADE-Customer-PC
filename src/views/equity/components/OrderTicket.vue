...
const algorithmValues = ['POV', 'DMA', 'none']
const algorithmDisplay = value => value === 'none' ? '无需算法' : value
const executionType = ref(props.initialOrder?.executionType === 'highTouch' || props.ticketContext?.executionType === 'highTouch' ? 'highTouch' : 'lowTouch'), algorithm = ref(algorithmValues.includes(props.initialOrder?.algorithm) ? props.initialOrder.algorithm : (algorithmValues.includes(props.ticketContext?.algorithm) ? props.ticketContext.algorithm : 'POV')), algorithmPopoverVisible = ref(false), orderType = ref(props.initialOrder?.type === 'market' ? 'market' : 'limit'), price = ref(props.initialOrder?.price), quantity = ref(props.initialOrder?.quantity), amount = ref(props.initialOrder?.amount), quantityMode = ref(props.initialOrder?.quantityMode || 'quantity'), fraction = ref(0), note = ref(props.initialOrder?.note || '')
...
const frameworkId = ref(props.initialOrder?.frameworkId || props.account.frameworks?.[0]?.id || '')
const instrumentSelect = ref(null), instrumentPopperWidth = ref(0)
const quantityInputKey = ref(0)
const displaySymbolCode = ref(props.initialOrder?.symbolCode || props.symbol.code)
...
const sourceOrderSide = computed(() => ['buy', 'sell'].includes(props.initialOrder?.sourceOrderSide) ? props.initialOrder.sourceOrderSide : null)
...
const accountFrameworks = computed(() => props.account.frameworks || [])
const selectedFramework = computed(() => accountFrameworks.value.find(item => item.id === frameworkId.value) || accountFrameworks.value[0] || null)
function accountOptionLabel(item) {
  const framework = item.id === props.account.id ? selectedFramework.value : item.frameworks?.[0]
  return `${item.id}·${item.name}${framework ? `【${framework.name}】` : ''}`
}
...
watch(() => props.initialOrder, value => {
  ...
  displaySymbolCode.value = value.symbolCode || props.symbol.code
  selectedManualInstrument.value = null
  executionType.value = value.executionType === 'highTouch' ? 'highTouch' : 'lowTouch'
  algorithm.value = algorithmValues.includes(value.algorithm) ? value.algorithm : 'POV'
  ...
  frameworkId.value = props.account.frameworks?.some(item => item.id === value.frameworkId) ? value.frameworkId : (props.account.frameworks?.[0]?.id || '')
...
watch(() => props.account.id, () => { frameworkId.value = props.account.frameworks?.[0]?.id || ''; resetOrder() })
...
function preview(side) {
  ...
  if (sourceOrderSide.value && side !== sourceOrderSide.value) return
  ...
  <div class="order-form" :class="{ 'is-paused': paused, 'is-manual': !closeMode && executionType === 'highTouch' }" :inert="paused ? '' : undefined" :aria-disabled="paused">
    ...
    <section v-if="!closeMode" class="order-context-block">
      ...
      <div class="order-account-block">
        ...
        <el-select :model-value="account.id" @visible-change="handleAccountVisible" @update:model-value="id => emit('account-select', id)" :aria-label="`下单账户 ${accountOptionLabel(account)}`" popper-class="variant-popper order-account-popper"><template #header><div class="account-dropdown-search" @click.stop><el-input v-model="accountSearch" placeholder="搜索下单账户" clearable /></div></template><el-option v-for="item in filteredAccounts" :key="item.id" :label="accountOptionLabel(item)" :value="item.id" /></el-select>
      </div>
      <div class="order-account-block order-framework-block"><div class="field-caption"><span>客户框架</span></div><el-select v-model="frameworkId" :disabled="!accountFrameworks.length" placeholder="请选择" aria-label="客户框架" popper-class="variant-popper"><el-option v-for="item in accountFrameworks" :key="item.id" :label="item.name" :value="item.id" /></el-select></div>
    ...
    <section class="quantity-block">
      ...
      <section v-if="!closeMode" class="order-account-block order-algorithm-block"><div class="field-caption"><span>算法</span></div><el-popover v-model:visible="algorithmPopoverVisible" trigger="click" placement="bottom-end" :width="264" popper-class="variant-popper algorithm-popper" @show="openAlgorithmConfig"><template #reference><button type="button" class="algorithm-trigger" :aria-label="`执行算法：${algorithmDisplay(algorithm)}，点击配置`"><span>{{ algorithmDisplay(algorithm) }}</span><svg class="algorithm-chevron" :class="{ 'is-open': algorithmPopoverVisible }" viewBox="0 0 12 12" fill="none" aria-hidden="true"><path d="m3 4.5 3 3 3-3" /></svg></button></template><div class="execution-algorithm-menu"><span>执行算法</span><el-radio-group v-model="draftAlgorithm" aria-label="执行算法"><el-radio-button label="POV">POV</el-radio-button><el-radio-button label="DMA">DMA</el-radio-button><el-radio-button label="none">无需算法</el-radio-button></el-radio-group><div v-if="draftAlgorithm === 'POV'" class="algorithm-popper-params" aria-label="POV 参数"><div class="algorithm-param-row"><span>运行时间</span><div class="algorithm-param-input"><el-select v-model="draftPovHours" class="algorithm-time-select" aria-label="POV运行小时" popper-class="variant-popper"><el-option v-for="hour in povHourOptions" :key="hour" :label="String(hour).padStart(2, '0')" :value="hour" /></el-select><i>时</i><el-select v-model="draftPovMinutes" class="algorithm-time-select" aria-label="POV运行分钟" popper-class="variant-popper"><el-option v-for="minute in povMinuteOptions" :key="minute" :label="String(minute).padStart(2, '0')" :value="minute" /></el-select><i>分</i></div></div><div class="algorithm-param-row"><span>市场成交占比</span><div class="algorithm-param-input is-participation"><el-input-number v-model="draftPovParticipation" :min="1" :max="25" :precision="0" :controls="false" aria-label="POV市场成交占比" /><i>%</i><small>不超过 25%</small></div></div><p v-if="draftPovParameterError" class="algorithm-param-error" role="alert">请填写有效的运行时间，市场成交占比不得超过 25%</p></div><footer class="algorithm-menu-actions"><el-button size="small" @click="cancelAlgorithmConfig">取消</el-button><el-button size="small" type="primary" :disabled="draftPovParameterError" @click="confirmAlgorithmConfig">确认</el-button></footer></div></el-popover></section>
