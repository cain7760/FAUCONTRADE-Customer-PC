...
function refreshHistory() { page.value = 1; refreshKey.value += 1 }
  ...
  <BaseDialog :model-value="modelValue" top="var(--ft-dialog-top)" width="min(1080px, calc(100vw - 32px))" :teleported="false" :show-close="false" class="cp-dialog history-dialog" :close-on-press-escape="true" @update:model-value="emit('update:modelValue', $event)">
    ...
    <template v-if="record">
      ...
      <section class="history-order-summary">
        ...
        <div class="history-summary-actions"><button type="button" class="history-refresh" aria-label="刷新历史记录" @click="refreshHistory"><el-icon><Refresh /></el-icon></button><button type="button" class="history-export" @click="exportHistory">导出历史</button></div>
      </section>
      <section class="history-records">
        <TradingTable :key="refreshKey" :data="pagedRecords" border class="history-records-table" height="100%"><el-table-column prop="operator" label="操作人" width="160" /><el-table-column prop="time" label="操作时间" width="220" /><el-table-column prop="type" label="操作类型" width="164" /><el-table-column prop="content" label="操作内容" min-width="360" /></TradingTable>
...
.history-summary-actions{position:relative;z-index:1;display:flex;align-items:center;gap:6px;flex:0 0 auto}.history-refresh{display:grid;width:28px;height:28px;padding:0;place-items:center;border:1px solid #4a5059;border-radius:3px;background:#2a2d32;color:#c8ced7;cursor:pointer}.history-refresh:hover{border-color:#bb984a;background:#353026;color:#e1c36f}@media(max-width:1000px){.history-summary-actions{margin-left:auto}}
</style>
