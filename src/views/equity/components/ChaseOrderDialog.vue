  ...
  <PositionOrderDialog :model-value="modelValue" title="追单" :position="position" :account="account" @update:model-value="emit('update:modelValue', $event)" @quote="quote = $event">
    ...
    <template #order>
      <form class="order-form chase-order-form" @submit.prevent="submit">
        ...
        <section class="quantity-block"><div class="field-caption"><label for="chase-quantity">订单数量</label></div><div class="quantity-input-row"><el-input-number id="chase-quantity" v-model="quantity" placeholder="请输入" :step="100" :precision="0" :controls="false" /><span>股</span></div><section class="order-account-block order-algorithm-block chase-algorithm-block"><div class="field-caption"><span>算法</span></div><el-popover placement="bottom-end" :width="174" trigger="click" popper-class="variant-popper algorithm-popper"><template #reference><button type="button" class="algorithm-trigger" :aria-label="`执行算法：${algorithm}，点击选择`"><span>{{ algorithm }}</span><svg class="algorithm-chevron" viewBox="0 0 16 16" aria-hidden="true"><path d="m4 6 4 4 4-4" /></svg></button></template><div class="execution-algorithm-menu"><span>选择执行算法</span><el-radio-group v-model="algorithm" size="small"><el-radio-button label="POV">POV</el-radio-button><el-radio-button label="DMA">DMA</el-radio-button></el-radio-group></div></el-popover></section><el-slider v-model="fraction" :step="1" :marks="{0:'0%',25:'25%',50:'50%',75:'75%',100:'100%'}" @input="size" /></section>
